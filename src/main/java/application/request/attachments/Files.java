...
public class Files {
    ...
    public static UploadFilesServer getWallUploadServer(PostModel postModel, User user) {
        Params params = Params.create()
                .add(AttachmentParams.USER_ID.getTitle(), postModel.getUser().getUserId().toString());
        String link = BaseRequestVK.baseRequestVK(AttachmentMethods.DOCS_GET_WALL_UPLOAD_SERVER.getTitle(), params, user);
    ...
    public static SerUploadFile getUpload(PostModel postModel, String path, AttachmentParams parameters, User user) {
    ...
    public static FileParam docsSave(PostModel postModel, SerUploadFile files, User user) {
        ...
        try {
            params = Params.create()
                    .add(AttachmentParams.FILE.getTitle(), URLEncoder.encode(files.getFile(), "UTF-8"));
        } catch (UnsupportedEncodingException e) {
            LOGGER.error("UnsupportedEncodingException", e);
        }
        link = BaseRequestVK.baseRequestVK(AttachmentMethods.DOCS_SAVE.getTitle(), params, user);
        JSONObject jsonResp = HttpConnection.getStringResponse(link);
        JSONObject jsonObject = (JSONObject) jsonResp.get("response");
